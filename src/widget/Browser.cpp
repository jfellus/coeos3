...
Browser::Browser(const std::string& server_id) : Widget(gtk_scrolled_window_new(NULL,NULL)), server_id(server_id) {
    ...
    g_signal_connect(G_OBJECT(webkitview), "document-load-finished", G_CALLBACK(on_load), this);
...
bool Browser::is_loaded() {
	return webkit_web_view_get_load_status(webkitview) == WEBKIT_LOAD_FINISHED;
}
