...
void PromNode::realize() {
	try {
		script_filename = file_dirname(net->filename) + "/" + properties["deploy"]->get_value_as_string() + "/" + properties["path_script"]->get_value_as_string();
		script = new PromScript();
		script->node = this;
		script->load(script_filename);

		script->name = properties["name"]->get_value_as_string();
		script->filename = properties["path_script"]->get_value_as_string();
	} catch(...) {script = NULL; }
}
